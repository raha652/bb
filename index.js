...
// این دو تابع رو اضافه کن (در بالای فایل index.js، بعد از generateId یا هر جا که خوبه)
function mapGSToUser(row) {
  return {
    __backendId: row.__backendId || generateId(),
    fullName: row.fullName || '',
    username: row.username || '',
    password: row.password || '',
    role: row.role || 'user',
    photo: row.photo || ''   // این خط مهم است!
  };
}
...
function mapUserToGS(user) {
  return {
    ...
    role: user.role,
    photo: user.photo || ''   // این خط مهم است!
  };
}
